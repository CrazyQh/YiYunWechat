...
!function (window) {
    ...
    CitySelect.prototype.open = function () {
        ...
        $cityElement.find('.cityselect-content').removeClass('cityselect-move-animate cityselect-next cityselect-prev cityselect-nextnextnext');

        _this.ForwardView4(false);
    ...
    CitySelect.prototype.createDOM = function () {
        ...
        _this.$cityElement = $('' +
            ...
            '        <p class="cityselect-title">房屋信息</p>' +
        ...
        _this.$cityElement.on('click.ydui.cityselect', '.cityselect-nav a', function () {
            ...
            if ($this.index() == 2) {
                _this.ForwardView2(true)
            }
            if ($this.index() == 3 || $this.index() == 4) {
                _this.ForwardView4(true)
            }
    ...
    CitySelect.prototype.setNavTxt = function (index, txt) {
        ...
        $nav.eq(index + 4).removeClass('crt').html('');
        $nav.eq(index + 5).removeClass('crt').html('');
    };

    CitySelect.prototype.backOffView = function () {
        this.$cityElement.find('.cityselect-content').removeClass('cityselect-move-animate cityselect-prev cityselect-nextnext cityselect-nextnextnext')
    ...
    CitySelect.prototype.ForwardView2 = function (animate) {
        this.$cityElement.find('.cityselect-content').removeClass('cityselect-move-animate cityselect-prev cityselect-nextnext cityselect-nextnextnext')
            .addClass((animate ? 'cityselect-move-animate' : '') + ' cityselect-nextnext');
    };
    CitySelect.prototype.ForwardView3 = function (animate) {
        this.$cityElement.find('.cityselect-content').removeClass('cityselect-move-animate cityselect-prev cityselect-nextnext cityselect-nextnextnext')
            .addClass((animate ? 'cityselect-move-animate' : '') + ' cityselect-nextnextnext');
    };
    CitySelect.prototype.ForwardView4 = function (animate) {
        this.$cityElement.find('.cityselect-content').removeClass('cityselect-nextnext cityselect-move-animate cityselect-prev cityselect-nextnext cityselect-nextnextnext')
    ...
    CitySelect.prototype.bindItemEvent = function () {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
            ...
            _this.setNavTxt(tag, $this.find("span").html());
            ...
            if (tag == 0) {
                ...
                _this.loadResident();
                _this.ForwardView3(true);
                $cityElement.find('.cityselect-item-box').eq(4).find('a').removeClass('crt');
            } else {
                //_this.loadResident();
                defaultSet.provance = $nav.eq(0).html();
                defaultSet.city = $nav.eq(1).html();
                defaultSet.area = $nav.eq(2).html();
                defaultSet.District = $nav.eq(3).html();
                defaultSet.Resident = $nav.eq(4).html();
                defaultSet.HouseID = $nav.eq(4).find('strong').html();
                //alert($nav.eq(4).find('strong').html())
                //alert($nav.eq(4).find('i').html())
                //alert(defaultSet.HouseID)
                $("#HouseCode").val($nav.eq(0).html() + "-" + $nav.eq(1).html() + "-" + $nav.eq(2).html() + "-" + $nav.eq(3).html() + "-" + $nav.eq(4).find('i').html())
                $("#HouseID").val($nav.eq(4).find('strong').html())
                _this.close();
            }
        });
    };
    ...
    CitySelect.prototype.returnValue = function () {
        ...
        _this.$element.trigger($.Event('done.ydui.cityselect', {
            ...
            District: defaultSet.District,
            Resident: defaultSet.Resident,
            HouseID: defaultSet.HouseID
        }));
    ...
    CitySelect.prototype.scrollPosition = function (index) {
        ...
        $itemBox.parent().animate({
            scrollTop: $itemBox.find('a.crt').index() * itemHeight - itemBoxHeight / 4
        }, 0, function () {
            _this.bindItemEvent();
        });
    };
    ...
    CitySelect.prototype.loadProvance = function () {
        ...
        $.ajax({
            type: "post",
            url: "/Owner/GetVillage",
            success: function (resutl) {
                $.each(JSON.parse(resutl), function (k, v) {
                    arr.push($('<a class= href="javascript:;"><span style="font-size:14px;">' + v.n + '</span><p style="display:none;">' + v.id + '</p></a>').data({
                        //citys: v.c,
                        tag: 0
                    }));
                });
    ...
    CitySelect.prototype.loadCity = function () {
        var _this = this;
        var VillageID = _this.$itemBox.eq(0).find('a.crt').find("p").text();
        var arr = [];
        $.ajax({
            type: "post",
            url: "/Owner/GetArea",
            ...
            success: function (result) {
                $.each(JSON.parse(result), function (k, v) {
                    arr.push($('<a class="' + (v.n == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span style="font-size:14px;">' + v.n + '</span><p style="display:none;">' + v.id + '</p></a>').data({
                        //citys: v.c,
                        tag: 1
                    }));
                });
    ...
    CitySelect.prototype.loadArea = function () {
        var _this = this;
        var AreaID = (_this.$itemBox.eq(1).find('a.crt').find("p").text())
        var arr = [];
        $.ajax({
            type: "post",
            url: "/Owner/GetFloor",
    ...
    CitySelect.prototype.loadDistrict = function () {
        var _this = this;
        var FloorID = (_this.$itemBox.eq(2).find('a.crt').find("p").text())
        var arr = [];
        $.ajax({
            type: "post",
            url: "/Owner/GetUnit",
    ...
    CitySelect.prototype.loadResident = function () {
        var _this = this;
        console.log(_this);
        var UnitID = (_this.$itemBox.eq(3).find('a.crt').find("p").text())
        var arr = [];
        $.ajax({
            type: "post",
            url: "/Owner/GetHouse",
            ...
            success: function (result) {
                $.each(JSON.parse(result), function (k, v) {
                    arr.push($('<a class="' + (v.n == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span style="font-size:14px;"><i>' + v.n + '</i><strong style="display:none;">' + v.id + '</strong></span><p style="display:none;">' + v.id + '</p></a>').data({

                        tag: 4
                    }));
                });
                _this.fillItems(4, arr);
            }
        })
    }
    ...
    //    var _this = this,
    //      $cityElement = _this.$cityElement;
    //    var areaData = (_this.$itemBox.eq(3).find('a.crt').find("p").text())

    //    $("#jiedao").text(areaData)
    //    //var arr = [];
    //    //$.ajax({
    //    //    type: "post",
    //    //    url: "/member/GetResidentList",
    //    //    data: { id: areaData },
    //    //    success: function (result) {

    //    //        if (result != "") {
    //    //            $.each(JSON.parse(result), function (k, v) {
    //    //                arr.push($('<a style="display: inline-block;" class="' + (v.n == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span style="font-size:14px;display: inline-block;height: 100%;">' + v.n + '</span><p style="display:none;">' + v.id + '</p></a>').data({
    ...
    //    //            defaultSet.District = $nav.eq(3).html();
    //    //            defaultSet.Resident = $nav.eq(4).html();
    //    //            $("#Address").val($nav.eq(0).html() + "-" + $nav.eq(1).html() + "-" + $nav.eq(2).html() + "-" + $nav.eq(3).html())
    //    //            _this.close();
    //    //        }
    //    //    }
    //    //})
